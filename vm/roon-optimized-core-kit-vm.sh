...
# Copyright (c) 2021-2025 inphinitum
# Author: inphinitum
# License: MIT
...
function header_info {
  clear
  cat <<"EOF"
   ___  ____  ____  _  __  ____       __  _       _            __  _____               __ ___ __ 
  / _ \/ __ \/ __ \/ |/ / / __ \___  / /_(_)_ _  (_)__ ___ ___/ / / ___/__  _______   / //_(_) /_
 / , _/ /_/ / /_/ /    / / /_/ / _ \/ __/ /  ' \/ /_ // -_) _  / / /__/ _ \/ __/ -_) / ,< / / __/
/_/|_|\____/\____/_/|_/  \____/ .__/\__/_/_/_/_/_//__/\__/\_,_/  \___/\___/_/  \__/ /_/|_/_/\__/ 
                             /_/                                                                 
EOF
}
header_info
...
function default_settings() {
  ...
  echo -e "${BL}Creating a ROCK VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 ROCK --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="ROCK"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create ROCK VM?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating a ROCK VM using the above advanced settings${CL}"
  else
    header_info
...
# Iterate over storage options and generate selection menu
...
# Stop|autoselect|prompt user about destination storage depending on available options none|one|multiple
...
# Available image is in RAW format, needs conversion for KVM
msg_info "Retrieving the URL for Roon Optimized Core Kit Disk Image"
URL=https://download.roonlabs.net/builds/roonbox-linuxx64-nuc4-usb-factoryreset.img.gz
sleep 2
msg_ok "${CL}${BL}${URL}${CL}"
...
msg_info "qm create $VMID -agent 1${MACHINE} -tablet 0 -localtime 1 -bios ovmf${CPU_TYPE} -cores $CORE_COUNT -memory $RAM_SIZE \
...
msg_info "pvesm alloc $STORAGE $VMID $DISK0 4M 1>&/dev/null"
msg_info "qm set $VMID \
...
if [ "$START_VM" == "yes" ]; then
  ...
  qm start $VMID
  msg_ok "Started Roon Optimized Core Kit VM"
fi
